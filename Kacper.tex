...
\begin{center}
Keywords: \textbf{multi-armed, bandit, active, semi-supervised, learning, 
...
in late 
60's, nevertheless, his work was overlooked for almost 20 years until it was revived in the early 80's.~\citep{gittins+
glazebrook+weber}
...
Less often we are interested in continuously developing events, that we went to discover or that require interaction. Simple statistics or probability might not be advanced enough to 
...
row of such machines can be transformed into only one automation but with multiple arms or buttons each corresponding 
...
be a good idea to employ some kind of a strategy that maximizes chances of 
...
becomes a serious dilemma that one might not have yet realized. The gambler 
faces a choice between the arm that has already been pulled with known sample expected return and any other arm which for now on remains a mystery, since there is no 
information about a potential reward.\\
If the gambler decides to take advantage of the already known arm and pulls it again, we 
...
memorize, for they extend to many real life applications. This section is devoted 
to a simple case study in which it seems natural to use the ``bandit approach''. 
Applications are versatile ranging from drug testing and maximizing income from 
...
cannot refuse to treat anyone, each person suffering from a disease(each 
\emph{play}) can be given two possible drugs(two \emph{arms}). The key 
...
The goal of a doctor is always to maximize the number of healed people. This model 
...
with the limited amount of scientists, time, or funding; the goal is to maximize the number of 
...
Then, we are moving to basic process description.   
...
goal is typically to recover unknown parameters that characterize stochastic 
...
The goal of agent is to maximize the overall reward from a game by following an optimal strategy...
...
\item the information exploitation i.e.  

information gathered after a \emph{play} can be used to modify chosen 
\emph{strategy}.
...
for an event occurring in non-deterministic intervals  
and information about the event can only be acquired during its occurrence.
\\
\end{itemize}
...
previous \emph{plays}; these bandits approaches are called the \emph{finite 
memory}.\\
We also assume that \emph{arms} are independent. This premise has recently been revised by~\citep{Pandey:2007:MBP:1273496.1273587}. 
MABs with dependent arms grow in popularity owing to the Internet advertising 
...
proportional to the original sequence.\\
...
bandits, each involving one known and one unknown arm.\\[2cm]



In the case of both sequences, the decision problem and the optimal strategy are 
...
An example of such sequence lets consider a $\gamma$ probability of terminating at each stage; therefore, 
...
all discount sequences''})$.\\

We may observe a discount sequence when we obtain both a reward and a value of the discount at a given stage. 
These cases may be harder to solve as we cannot replace random with a non-random sequences in random sequences without significantly altering the problem.
\\
...
directly influence the decision making process. To visualize this scenario we should
...
case, wheareas a mean maximizing action can be a good choice in the second case.\\
...
sequence indicates that the first $n-1$ stages are played for a sole purpose of 
...
Once we can formally define the MAB problem we are in the position to start seeking an optimal strategy. There are numerous different approaches available all with different pros and cons.\\
\section{Seeking an optimal solution}
Strategies presented below are used to balance exploration and exploitation to 
...
to gain more information about the system and maximize the future return.\\
We could also democratize such a dramatic scenario by using a geometric sequence---in this way, the health of current patients 
...
state against some standardized actions with simple properties. The main 
...
set of all available alternatives; the selection process is then maximizing this 
function.\\
...
decision processes. The \emph{index theorem} states advocates the existence of an optimal \emph{index policy}, 
...
\lambda_t \mathscr{R}(h)$\;\label{al:LCB-AL:LCB}
 ...
 \caption{LCB-AL presented in~\citep{DBLP:journals/corr/GantiG13}.\label{al:LCB-
AL}}
\end{algorithm}
\vspace{2cm}
...
To begin with, no other steps  but line~\ref{al:LCB-
AL:LCB} of the algorithm  ~\ref{al:LCB-AL} will change. As it was mentioned above, line~\ref{al:LCB-AL:LCB} is 
...
To test and evaluate our version of active learning MAB algorithm(\ref{al:LCB-
AL} \& \ref{al:TS-AL}) presented in chapter~\ref{ch:MAB-AL} we conducted an 
