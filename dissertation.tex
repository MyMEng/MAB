...
Thompson Sampling was first proposed in 1933 by William R.\ Thompson~\cite{thompson:biom33}. In this section we explain this approach and discuss related work extending originally proposed methodology~\cite{May:2012:OBS:2503308.2343711}.\\
Firstly the general idea is presented. 

General idea:\\
Thompson sampling takes a sample from posterior distribution of all actions and decide on the one that indicates maximal reward.\\
This variance minimization leads to exploring actions which in general give high value but once a tiem can give largest value due to high variance.\\
With variance minimization we arrive with better and better and better estimation of true mean with time leading to decision being less and less unbiased.\\
...
Usually the sample will be quite close to mean~\ref{fig:sim1} (normal distribution) yielding choice of action with highest mean but occasionally it will lie in the tail leading to exploration of suboptimal action at given time~\ref{fig:sim2}. This argument intuitively guarantees infinite exploration of environment thus convergence to overall optimal solution.
...
To utilize introduced above approach
...
There is a number of historical applications of multi-armed bandits (MAB) mainly concerned with research time management, general job-scheduling, economics and military~\cite{gittins+glazebrook+weber}.
