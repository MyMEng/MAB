...
There is a number of historical applications of multi-armed bandits(MAB) concerned mainly with research time management 
Nowadays main use is found in contextual bandits which are foundations of optimizing online advertising income by different variations of ads layout at given webpage widely used by IT giants like Google, Yahoo, Facebook, LinkedIn or Microsoft. For majority of this companies this theory is of high interest as lion's share of their revenue flows from click-through rate in online advertising.~\cite{graepel2010web} and google paper\\

Other, most resent application of MAB theory fitted into computer science world is concerned with classification issue in environment of insufficient information. This application in machine learning field seems to be under-explored are which so far was addressed in one scientific publication~\cite{DBLP:journals/corr/GantiG13}.\\
Our decision was to pursue this topic mainly by presenting here deep analysis of chosen in \cite{DBLP:journals/corr/GantiG13} approach. After full exploration we seek for potential improvements and alternatives.

\section{Structure of chapter}
We first introduce foundations of machine learning needed to develop concepts of semi-supervised and active learning. Next, we introduce whole family of active learning(AL) scenarios and give detailed description of chosen one. Then we present possible connection between processes of MAB and AL. This interface, will allow to frame active learning scenario as bandit process.\\
Proceeding, main section of this chapter focuses on described in \cite{DBLP:journals/corr/GantiG13} ``briddge'' connecting both worlds. Finally, we explore potential changes that can be introduced and improvements that can be beneficial for described earlier algorithm.
%explore in great detail

\section{Machine Learning 101}
ML~\cite{flach2012machine}.\\
Machine Learning is a field of Computer Science which is mainly concerned with building a model for a given data. Supplied information are of form $2$

-------
The perfomance throug error data


There are a few main settings of machine learning which can be divided by two criteria. Namely the form of supplied training data: whether it is labeled --- \emph{supervised} learning or unlabeled --- \emph{unsupervised} learning and the goal of classification: whether process aims at predicting a target variable(label) --- \emph{predictive} model or discover the structure of the data(describe) --- \emph{descriptive} model.\\

------


\subsection{Semi-supervised Learning}
There is a missing link in the chain which, connects supervised and unsupervised scenarios called \emph{semi-supervised} learning. The goal of such classification is to train classifier on relatively small sample of labeled data in a way that its performance on unseen data is close to one that we could achieve with supervised algorithm and huge sample of labeled data. The general strategy is to use the most informative points which describe underlying distribution behind each class.\\

The main application area of semi-supervised learning is natural language, video or audio processing and other fields where data are relatively easy to acquire --- for instance by placing camera in a public place, hours of material can be recorded --- but the process of labeling gathered data to find good fitting supervised model is expensive, time consuming or demand hours of manual labour.\\
