...
% Title
\HRule \\[0.4cm]
{ \huge \bfseries \emph{\textbf{Multi-armed bandits}}\\[0.5cm]
	Comprehensive introduction with focus on \emph{Thompson Sampling}\\
  \&\\
...
\begin{abstract}
\thispagestyle{empty}% no header and footer
This dissertation covers two main topics: multi-armed bandits theory with extensive treatment of Thompson Sampling approach as well as application of MAB in active learning. The comprehensive introduction to theory underlying multi-armed bandits is gradually developing concepts necessary for understanding the basic bandit strategies. The late part of this work presents first of its kind(as far as we know) application of MAB algorithm to solve computer science task of learning in environment with insufficient information.\\
Reader is assumed not to need any prior knowledge in this field, only basics of statistics and probability theory are required to smoothly follow the text.
\begin{center}
