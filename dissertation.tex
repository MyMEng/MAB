...
where:
\begin{description}
\item[$p \left( \mathrm{B} | \mathrm{A} \right)$] is \textbf{posterior}
\item[$p \left( \mathrm{A} | \mathrm{B} \right)$] is \textbf{likelihood}
\item[$p \left( \mathrm{B} \right)$] is \textbf{prior}
\item[$p \left( \mathrm{A} \right)$] is \textbf{normalizing factor}
\end{description}

The theorem in plain English states that .
