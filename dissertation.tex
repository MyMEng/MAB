...
Other, most resent application of MAB theory fitted into computer science world is concerned with classification issue in environment of insufficient information. This approach to machine learning field seems to be under-explored and needs more attention from a research world as to the best of my knowledge it was addressed so far in only one scientific publication, namely~\cite{DBLP:journals/corr/GantiG13}.\\
...
Such function $\mathit{l}$ is called \emph{model}, which we try to approximate. The instances which are supplied to the model are preprocessed to extract \emph{features}, which represent each instance in a form of vector describing chosen attributes. Based on feature vector given model outputs the element of output space.\\
...
In general supervised learning is one of the most common scenarios and is concerned with finding a \emph{class} for a given exemplar. The most common example is to build a model which predicts whether a particular e-mail is spam or non-spam based on extracted features i.e.\ words from the content of the message.\\

Another important building block of machine learning theory is the concept of evaluating accuracy or performance of a trained model. The most popular measure in presented above supervised scenario is so called error rate. It is calculated using \emph{test set} which is disjoint with training set i.e.\ unseen by trained model. Typically the value is calculated as $\mathtt{Err} = \frac{\text{correctly classified instances}}{\text{all instances}}$.\\

The main goal of machine learning is to choose some \emph{hypothesis} from \emph{hypothesis space} which best ``fits'' given data sample. In general hypothesis space is a set of all possible assignments of classes to data points. For instance if in order three data points $(x_1, x_2, x_3)$ and our target is binary classification i.e.\ we either predict $+$ or $-$, our hypothesis space is then $H = {(+, +, +), (+, +, -), (+, -, +), (-, +, +), (+, -, -), (-, +, -), (-, -, +), (-, -, -)}$.\\
If we discuss performance of given hypothesis more sophisticated measure of uncertainty is \emph{loss function} $L : \mathbb{R} \rightarrow [0, \infty)$ which maps \emph{margin} $z(\mathbf{x})$ of particular exemplar to an associated loss $L(z(\mathbf{x}))$. \\
...
Is a special case of SSL where the agent has access to \emph{label oracle} $\mathscr{O}$.
