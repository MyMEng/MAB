...
The most common distribution in statistics with well known bell-shaped plot is the normal distribution also called \emph{Gaussian} distribution. If a random variable $X$ follow such distribution parametrized by \emph{mean} $\mu$ and \emph{standard deviation} $\sigma$ we commonly write $X \sim \mathcal{N}\left( \mu, \sigma \right)$.

Probability density function
with area under curve from $-\infty$ to $+\infty$ integrating to $1$.
$$
