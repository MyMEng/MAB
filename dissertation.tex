...
To build a bridge between MAB and AL we first need to describe setting of such scenario. For the sake of simplicity we focus on pool based binary active learning which is based on search through \emph{hypothesis space} $\mathscr{H}$ with query budget $B$. The key point is to view learning problem as exploration-exploitation trade-off. We will firs focus on finding connections between characteristic features of MAB and try to find correspondence in AL.\\

To begin with, we will frame learning problem as $B$ round game --- finite horizon with discount sequence $\mathbf{A} = ( \underbrace{ 1, 1, 1, ..., 1}_{B}, 0, 0, 0, ... )$ --- where in each round agent is faced with pulling on of given arms and suffer some loss $L_t$ on chosen action. For simplicity, in this scenario we change reward for a loss what leads to optimal strategy being the one that minimizes loss instead of maximizing reward over $B$ rounds.\\
To be more precise, we define equivalence between arms of a bandit and hypothesis $\mathit{h} \in \mathscr{H}$. We also need to find a proxy of MAB loss signal in our active learning scenario. To define the later lets first clearly state goals of our strategy.\\
The main aim is to estimate optimal hypothesis i.e.\ the one with lowest risk with use of as little labeled points as possible. If we knew best $\mathit{h}$, so the one with minimal cumulative loss then the optimal strategy would be to pull this arm in each round of our experiment. This leads to the goal of finding optimal strategy as quickly as possible. Now we are left with defining a strategy telling us which arm to pull in each round and how to transform feedback (label) of querying a point $x$ from our pull into a loss signal.\\
To address the first mentioned issue 
...
Define $\mathscr{Q}_t$ a set of labels $y_i$ acquired by queering $\mathscr{O}$ for label of $x_i$ up to time $t$.\\
Just to recall we handle a binary classification so set $y_i$ to $+1$ for positive class and $-1$ for negative class.\\

To decide which point to query we utilize introduced earlier loss and margin function. Based on~\cite{DBLP:journals/corr/GantiG13} we define margin $z(x_i) = \hat{c}(x_i)h_t(x_i)$ where 
$\hat{c}(x_i) =   \begin{cases}
                  c(x_i) & \text{if } x_i \in \mathscr{Q}_{t-1} \\
                  h_t(x_i) & \text{otherwise}
                \end{cases}$.
Moreover, set $h(x_i) =   \begin{cases}
                  +1 & \text{if $h_t$ predicts $x_i$ to belong to positive class} \\
                  -1 & \text{otherwise}
                \end{cases}$.\\
...
In our opinion there is still some room for improvement here which is described in Section:~\ref{sec:qimprove}.\\
...
To restrict hypothesis space so to work with finite set we use self-concordant vulgarizers $\mathscr{R}(h)$ defined in~\cite{Abernethy08competingin} as:
$$
\mathscr{H} = \{ h : ||h|| \leq R \}
$$
$$
\text{where } L_2 \text{ norm}
$$
$$
||h||_x = \sqrt{ \frac{1}{ <h, h>_x } }
$$
$$
...
$$
$$
\mathscr{R} = - \log(R^2 - ||h||^2)
$$\\

If we choose set of linear classifiers the linear classifier is defined as:
$$
c(x) = w \cdot x - t
$$
...
Linear model(hypothesis) is finding a weight vector $w$ such that our classifier is of the form given above. As we can express decision boundary as $\mathbf{w} \cdot \mathbf{x} = t$. Our decision boundory is a plane in the cpace spanned by $x_i$. The vector $w$ is perpendicular to boundary and points in $+$ class direction.\\
The notation of classifier can be simplified by extending both vectors $x$ and $w$ with $x_0 = 1$ and $w_0 = -t$ leading to $c(x) = w^{\circ} \cdot x^{\circ}$\\

Please see figure~\ref{fig:binclas}.\\

Then we can denote our hypothesis space as span of vector $w$.\\




What is self-concordant regularizer?\\
S-CB allows us to reduce dimentionality of hypothesis space by .\\
modeling such infinite or huge hypothesis space woud suffer from many arms in bandits --- effiviency issue.\\


According to paper the cumulative loss suffered through game for arm $a$ at time $t$ is:
$$
\hat{L}_t(a) = \frac{1}{nt} \sum_{i=1}^{n} \sum_{\tau = 1}^{t} \frac{Q^{\tau}_i}{p^{\tau}_i} L(c_i h(x_i))
$$
where $p^t_i$ is probability of querying $x_i$ in round $t$ extracted from vector described above.\\
Furthermore $Q^t_i$ is a random variable which takes values $\{0,1\}$; $1$ if $x_i$ was queried in round $t$ and $0$ otherwise.\\
Such unbiased estimator of hypothesis risk was first proposed by~\cite{journals/jmlr/GantiG12} and in our case can be interpreted as an estimator of cumulative loss the agent would have suffer by pulling arm $a$ for the first $t$ rounds.\\
...
??$p_{\text{min}}$??\\
$p_{\text{min}}$ is parameter which defines the minimum probability of any point to be queried.\\

??$L^2_{\text{max}}$??\\
For the loss function we assume that it is convex and is upper bounded by some $L^2_{\text{max}} < \infty$ --- it can eb logistic, exponential, squared.\\

??$\delta$??\\
$\delta$ is a parameter which is bounded by $\delta < \frac{1}{e}$.\\
