...
In this section we compare results achieved above for Thompson Sampling policy against ones, achieved via \emph{Lower Confidence Bound} selection mechanism---straightforward application of algorithms~\ref{al:LCB-AL}. We conducted experiments for the same query budget as above: 1\%, 5\%, 10\%, 15\% and 20\%.\\
...
For 20\% sample the LCB converges in similarly to algorithm presented above. Optimal hypothesis is chosen at the end and is very close to second optimal. Above we have two ``75\% correct'' hypothesis, and at the top(the worst) two arms which assigned singleton class. The only advantage of LCB is \textcolor{red}{red} being in top rather than middle.\\
Here, same as above 1\% sample behaves random and does not contain any informative content.\\
