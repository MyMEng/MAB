...
\item Baesian's statistics.
\end{itemize}
It is possible to use any prior and likelihood distribution with \emph{Thompson Sampling} but for the sake of simplicity we will focus here on \emph{normally distributed} \textit{prior} and \textit{likelihood}. This restriction does not mean that it is not possible to do it with other distributions but it would demand from reader to be familiar with analytic approach to Baesian's \emph{posteriors} with regard to chosen \emph{prior} and \emph{likelihood}.
...
\subsection{\emph{prior}, \emph{posterior} and \emph{likelihood} distributions}
...
Basic results that yields from Bayesian statistics for normal distribution~\cite{gelman2003bayesian}\cite{lynch2007introduction} 
