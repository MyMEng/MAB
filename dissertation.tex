...
\textbf{\textrm{Known variance. }}Firstly we will consider \emph{normal prior--normal likelihood with $\sigma^2$ known}.
$$
f \left( \mu | \mathrm{X} \right) \propto f \left( \mathrm{X} | \mu \right) f \left( \mu \right)
$$
The $\sigma^2$ in the notation is omitted for clarity purposes.\\


\textbf{\textrm{Unknown variance. }}In both later cases the posterior is normally distributed.\\
...
In our approach we use sampling to draw at random .
